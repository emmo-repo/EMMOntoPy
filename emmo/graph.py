...
class Graph:
    """A class used by emmo.ontology.Ontology that adds functionality for
    generating graph representations of the ontology.
    """
    _default_style = {
        'graph': {
            'graph_type': 'digraph',
        ...
        'node_properties': {
            'defined': {'fillcolor': '#ffc880'},
            'individual': {'fillcolor': '#80c8ff', 'shape': 'diamond'},
            'class_construct': {'shape': 'box'},
            # Whether to add a second line to the label with relation charac-
            # teristics:
            #   F (functional), IF (inverse functional), T (transitive),
            #   S (symmetric), AS (asymmetric), R (reflexive), IR (irreflexive)
            'show_characteristics': True,
        },
        'edge': {},
        'edge_properties': {
            # OWL relations (properties)
            'is_a': {'arrowhead': 'empty'},
            'equivalent_to': {'color': 'green3', },
            'disjoint': None,
            'argument': {'color': 'red', },  # class construct argument
            'inverse_of': {'color': 'orange', },  # between relations

            # Ontology-specific relations
            'mereotopology': {'color': 'red', 'arrowtail': 'diamond',
                          'dir': 'back'},
            'semiotic': {'color': 'blue', 'style': 'dotted'},
            'set_theory': {'color': 'magenta'},
    ...
    def __init__(self, classes=None, edges=None, style=None, **kw):
        """Creates a Graphviz graph with the given nodes and edges."""
        self.style = style if style else self._default_style.copy()
        self.graph = Digraph(**kw)
        self.classes = []
        if classes:
    ...
    def add_classes(self, classes, style=None):
        """Adds `classes` to graph."""
        if isinstance(classes, (owlready2.ThingClass, owlready2.Thing,
...
class ClassGraph(Graph):
    """A graph subplass for plotting classes and relations between them."""
    def __init__(self, classes, relations=None, style=None):

        super().__init__(nodes=classes, edges=relations, style=style)
