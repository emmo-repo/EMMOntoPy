...
class OntoDoc:
    ...
    def get_vocabulary(self, items=None, sections=None, chapter=None,
        ...
        """
        warnings.warn('get_vocabulary() is deprecated.  Use the DocPP class '
        ...
        if sections is None:
            if items is None:
                relations = self.get_vocabulary(
                    items=onto.object_properties(), template=template)
                entities = self.get_vocabulary(
                    items=onto.classes(), template=template)
                individuals = self.get_vocabulary(
                    items=onto.individuals(), template=template)
        ...
        for header, ingress in sections.items():
            if items is None:
                items = onto.get_branch(header, sections, include_leafs=False)
            ...
            for item in items:
                ...
                # Add annotations
                annotations = onto.get_annotations(item)
                for key in sorted(annotations.keys(), key=sorter):
                    for value in annotations[key]:
                        for reg, sub in template.get('substitutions', []):
                            value = re.sub(reg, sub, value)
                        lannotations.append(annotation_template.format(
                            key=key.capitalize(), value=value, ontology=onto))

                # ...add iri
                lannotations.append(annotation_template.format(
                    key='IRI', value=asstring(item.iri, link), ontology=onto))
                ...
                for p in item.is_a:
                    if (isinstance(p, nonProp) or
                        (isinstance(item, owlready2.PropertyClass) and
                         isinstance(p, owlready2.PropertyClass))):
                        points.append(point_template.format(
                            point='is_a ' + asstring(p, link), ontology=onto))
                    else:
                        points.append(point_template.format(
                            point=asstring(p, link), ontology=onto))
                ...
                if show_individuals and hasattr(item, 'instances'):
                    ...
                    for e in item.instances():
                        points.append(point_template.format(
                            point=asstring(e, link), ontology=onto))
                    if points:
                        value = points_template.format(
                            points=''.join(points), ontology=onto)
                        lannotations.append(annotation_template.format(
    ...
    #          **kwargs):
    #    """Writes controlled vocabulary to `outfile`.
    #
    #    Args:
    #        outfile: File that the documentation is written to.
    #        format: Output format.  If it is "md" or "simple-html",
    ...
    #        pandoc_args: Sequence with command line arguments provided to
    #            pandoc.
    #        genfile: Store temporary generated markdown input file to pandoc
    #            to this file (for debugging).
    #        **kwargs: Keyword argiments passed to the get_vocabulary() method.
    #    """
    #    run_pandoc = False
    #    if format is None:
    ...
    #        kwargs.setdefault('template', 'html')
    #    elif format == 'md':
    #        kwargs.setdefault('template', 'markdown')
    #    else:
    #        kwargs.setdefault('template', 'markdown')
    #        run_pandoc = True
    #
    #    content = self.get_vocabulary(**kwargs)
    #
    #    if run_pandoc:
    #        if not genfile:
    ...
    #                f.write(content)
    #        self.run_pandoc(genfile, outfile, format, pandoc_args=pandoc_args)
    #    else:
    #        with open(outfile, 'wt') as f:
    #            f.write(content)

    #def map_unicode(self, infile, outfile=None):
    #    """Reads `infile`, maps unicode characters to latex and writes the
    #    result to `outfile`.  If `outfile` is not provided, `infile` is
    #    overwritten."""
    #    # This list may be extended after needs
    #    mappings = {
    #        '∃': r'$\exists$',
    #        '∀': r'$\forall$',
    #        '∧': r'$\land$',
    ...
    #def generate_document(self, outfile, introduction_md=False,
    #                      relations_md=None, classes_md=None,
    #                      individuals_md=None, appendix_md=None):
    #    """Generates a markdown document.
    #
    #    The generated document have five chapters; introduction, relations,
    #    classes, individuals and appendix.
    #
    #    For each chapter, one can provide a markdown file, controlled via the
    #    corresponding `*_md` arguments.  If they are:
    #      :None: The default file `*.md` will be used if it exists, otherwise
    #           a minimal default will be used.
    #      :True: Use minimal default.
    #      :False: Skip section.
    #      :filename: Use this file.
    #    """
    #
    #    def load_chapter(name, md):
    #        """Returns content of chapter template."""
    #        mdfile = name + '.md'
    #        if md is False:
    #            return ''
    #        if md is None:
    #            md = mdfile if os.path.exists(mdfile) else True
    #
    #        elif md is True:
    #            content = ''
    #
    #    # Chapter 1 - introduction
    #    if introduction_md is None:
    #        if os.path.exists('introduction.md'):
    #            introduction_md = 'introduction.md'
    #        else:
    #            introduction_md = True
    #    if introduction_md is False:
    #        pass
    #    elif introduction_md is True:
    #        pass
    #    else:

    #def load_pandoc_options(self, yamlfile):
    ...
    #    return args
    #
    #def run_pandoc(self, genfile, outfile, format, pandoc_args=[],
    #               verbose=True):
    #    """Runs pandoc.
    #
    #    Parameters
    ...
    #            args.extend(self.load_pandoc_options(fname))
    #        else:
