...
with onto:
    ...
    for Z, (symbol, name, mass) in enumerate(
        zip(
        ...
        Element.elucidation.append(
            en(f"The symbol that stands for the {name.lower()} atom.")
        )
...
onto.metadata.comment.append(
    en("You can contact EMMO Authors via emmo@emmc.eu")
)
...
d = {
    o.base_iri.rstrip("/#"): o.get_version(as_iri=True)
...
for abbrev_iri in onto.world._get_obj_triples_sp_o(
    ...
    version_iri = d[iri]
    onto._del_obj_triple_spo(onto.storid, owlready2.owl_imports, abbrev_iri)
    onto._add_obj_triple_spo(
        onto.storid, owlready2.owl_imports, onto._abbreviate(version_iri)
    )
