...
class Ontology(owlready2.Ontology):  # pylint: disable=too-many-public-methods
    ...
    def get_by_label(
        ...
        if prefix:
            ...
            if len(entitylist) > 0:
                if len(entitylist) > 1:
                    warnings.warn(
                        f"Several entities with label {label!r} "
                        f"and prefix {prefix!r} found. "
    ...
    def get_by_label_all(self, label, label_annotations=None, prefix=None):
        ...
        # if entity_accessed_directly and entity_accessed_directly not in entities:
        #    entities.append(entity_accessed_directly)

        # Find existece in get_entities
