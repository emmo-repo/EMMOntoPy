...
class Ontology(owlready2.Ontology, OntoGraph):
    ...
    def __dir__(self):
        ...
        if self._dir_preflabel:
            s.update(
                e.prefLabel.first() for e in lst if hasattr(e, "prefLabel")
            )
        if self._dir_label:
    ...
    def get_by_label(self, label, label_annotations=None, namespace=None):
        ...
        if "namespaces" in self.__dict__:
            if namespace:
                ...
                raise NoSuchLabelError(
                    'No label annotations matches "%s" in namespace '
                    '"%s"' % (label, namespace)
                )
        ...
        if label_annotations is None:
            ...
            annotations = (
                s.name if hasattr(s, "storid") else s for s in label_annotations
            )
    ...
    def sync_reasoner(
        ...
        if reasoner == "Pellet":
            ...
            raise ValueError(
                'unknown reasoner %r. Supported reasoners are "Pellet", '
                '"HermiT" and "FaCT++".',
                reasoner,
            )
    ...
    def get_annotations(self, entity):
        ...
        for a in self.annotation_properties():
            d[a.label.first()] = [
                o.strip('"')
                for s, p, o in self.get_triples(entity.storid, a.storid, None)
            ]
        return d
    ...
    def get_branch(
        ...
        if sort:
            branch = sorted(
                sorted(branch, key=lambda x: asstring(x)),
    ...
    def _number_of_generations(self, descendant, ancestor, n):
        ...
        if descendant.name == ancestor.name:
            return n
        return min(
            self._number_of_generations(parent, ancestor, n + 1)
    ...
    def get_ancestors(self, classes, include="all", strict=True):
        ...
        if include == "all":
            ...
            for c in classes:
                ancestors.update(
                    a for a in c.ancestors() if closest in a.ancestors()
                )
